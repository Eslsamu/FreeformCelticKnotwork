...
from atomic_meshing import Atomic_Mesh_Generator
...
def draw_vertex(t,x,y):
    ...
    #t.goto(x,y)
    #t.dot(10,"red")
    t.create_oval(x,y,x,y,width=10,fill= 'white')
    #t.home()
...
def view_optimization():
    ...
    init = mesh_init.init_pixels_go('eyeball.png')
    print(init)
    msg = ' '.join(str(x) for x in init)
    popen = subprocess.Popen(['go run atomic_meshing.go ' +
                              msg] ,
                             shell=True, stdout=subprocess.PIPE,
                             stderr=subprocess.STDOUT)
    output = popen.stdout.read()
    result = json.loads(output)
    atoms = np.reshape(result["X"],(len(init),2))

    #load = Image.fromarray(mask)
    #render = ImageTk.PhotoImage(load)

    #canvas.create_image(10,10,anchor=NW,image=render)
    canvas.pack(fill=BOTH, expand=1)

    graph["vertices"] = atoms
